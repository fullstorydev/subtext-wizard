...
/**
 * Parse argv, validate it, and hand off to the wizard. Exits the process on
 * its own (parse error, unsupported Node, wizard result) exactly as before —
 * it lives here rather than in bin.ts only so tests can import it without
 * running a wizard as a side effect of the import.
 */
